      ...
      <div class="box-body">
        <form action="{{ url("/GuideCalling/export") }}" method="post" id="frm" name="frm">
          ...
          <div class="row">
            <table width="95%" cellspacing="2" cellpadding="2" border="0">
              <tr>
                <th colspan="5" align="left">Total <span style="color:#F00">{{ count($quote_data) }}</span>  record(s)</th>
              </tr>
              <tr>
                <th bgcolor="#CCCCCC">Quotation Code</th>
                <th bgcolor="#CCCCCC">Quotation Name</th>
                <th bgcolor="#CCCCCC">Valid From</th>
                <th bgcolor="#CCCCCC">Valid To</th>
              </tr>

              @foreach ($quote_data as $data)
              <tr>
                <td height="23" align="center">{{ $data->QuotationCode }}</td>
                <td align="left">{{ $data->QuotationName }}</td>
                <td align="center">{{ $data->validFrom }}</td>
                <td align="center">{{ $data->validTo }}</td>
              </tr>
              @endforeach
